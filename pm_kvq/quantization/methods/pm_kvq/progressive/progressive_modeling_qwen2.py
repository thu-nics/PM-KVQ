...
def Progressive_Qwen2Attention_forward(
    self: Qwen2Attention,
    ...
    if past_key_value is not None:
        ...
        if kv_memory > self.kv_budget:
            if current_key_value_len <= self.n_sink_token_in_memory + self.n_window_token_in_memory:
                raise MemoryError(f"Memory Error: The kv cache is full, please reduce the kv cache size or increase the kv budget. Current kv budget: {self.kv_budget}, current kv cache size: {kv_memory}")
            else:
                self.n_kv_bits //= 2
                if self.n_kv_bits < 2:
                    raise MemoryError(f"The kv bits is too small, please increase the kv budget or reduce the kv cache size. Current kv budget: {self.kv_budget}, current kv cache size: {kv_memory}")
